...
use symfony\yaml\Exception\DumpException;
            ...
            while ($i < $len) {
                ...
                switch ($mapping[$i]) {
                    case '[':
                        ...
                        if (isset($output[$key])) {
                            @trigger_error(sprintf('Duplicate key "%s" detected on line %d whilst parsing YAML. Silent handling of duplicate mapping keys in YAML is deprecated since version 3.2 and will throw \symfony\yaml\Exception\ParseException in 4.0.', $key, self::$parsedLineNumber + 1), E_USER_DEPRECATED);
                            $duplicate = true;
                        }
                        break;
                    case '{':
