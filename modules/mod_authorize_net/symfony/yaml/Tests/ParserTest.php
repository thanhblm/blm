...
EOF;
    ...
    {
        $input = <<<'EOF'
foo: !php/object:O:30:"symfony\yaml\Tests\B":1:{s:1:"b";s:3:"foo";}
bar: 1
EOF;
...
YAML;
    ...
    /**
     * @dataProvider invalidDumpedObjectProvider
     * @expectedException \symfony\yaml\Exception\ParseException
     */
    ...
    public function testNonUtf8Exception()
    {
        ...
        foreach ($yamls as $yaml) {
            try {
                ...
                $this->assertInstanceOf('symfony\yaml\Exception\ParseException', $e, 'charsets other than UTF-8 are rejected.');
            }
        }
    }
...
EOD;
     ...
     * throws \symfony\yaml\Exception\ParseException in 4.0
     */
...
EOF;
    ...
    /**
     * @group legacy
     * @expectedDeprecation Implicit casting of incompatible mapping keys to strings is deprecated since version 3.3 and will throw \symfony\yaml\Exception\ParseException in 4.0. Pass the PARSE_KEYS_AS_STRING flag to explicitly enable the type casts.
     */
...
EOT;
    ...
    {
        if (method_exists($this, 'expectException')) {
            $this->expectException('\symfony\yaml\Exception\ParseException');
            $this->expectExceptionMessage(sprintf('Unexpected characters near "," at line %d (near "bar: "123",").', $lineNumber));
        } else {
            $this->setExpectedException('\symfony\yaml\Exception\ParseException', sprintf('Unexpected characters near "," at line %d (near "bar: "123",").', $lineNumber));
        }
...
YAML
    ...
    /**
     * @group legacy
     * @expectedDeprecation Starting an unquoted string with a question mark followed by a space is deprecated since version 3.3 and will throw \symfony\yaml\Exception\ParseException in 4.0.
     */
