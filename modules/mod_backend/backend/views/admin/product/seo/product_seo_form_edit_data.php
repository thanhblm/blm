...
$productSeos = RequestUtil::get('productSeos');
$languages = $productSeos->getArray();
?>
		...
		<div class="tab-content">
			...
			if (! empty ( $languages ) && count ( $languages ) > 0) {
				...
				<div class="tab-pane fade <?=$i==0?"active in":""?>" id="seo_<?=$lang->languageCode ?>" data-repeater-item>
						<div class="form-body">
							<input type="hidden" name="productSeos[<?=$i?>][flag]" value="<?=$lang->flag?>">
							<input type="hidden" name="productSeos[<?=$i?>][languageName]" value="<?=$lang->languageName?>">
							<input type="hidden" name="productSeos[<?=$i?>][languageCode]" value="<?=$lang->languageCode?>">
							<input type="hidden" name="productSeos[<?=$i?>][itemId]" value="<?=$lang->itemId?>">
						<?php
						$text = new TextInput ("text_input_fluid");
						$text->label = Lang::get ( "Url" );
						$text->name = "productSeos[$i][url]";
						$text->value = $lang->url;
						$text->render ();
						
						$text = new TextInput ("text_input_fluid");
						$text->label = Lang::get ( "Title" );
						$text->name = "productSeos[$i][title]";
						$text->value = $lang->title;
						$text->render ();
						
						$text = new TextInput ("text_input_fluid");
						$text->label = Lang::get ( 'Keyword' );
						$text->name = "productSeos[$i][keywords]";
						$text->value = $lang->keywords;
						$text->render ();
							
						$textArea = new TextArea ("textarea_fluid");
						$textArea->label = Lang::get ( 'Description' );
						$textArea->name = "productSeos[$i][description]";
